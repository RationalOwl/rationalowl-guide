...
|------------- | ------------- | ------|
|실시간 다운스트림 메시지 평균 전송 시간  | 0.2초  | 1초 내외|
|업스트림 메시지 수신  | O  | X|
...
- 단말앱이 실행시에 평균 0.2초의 실시간 데이터 전달의 용도로 이용한다.
...
앱서버 라이브러리를 이용할 경우 다운스트림 메시지 전달시간은 평균 0.2초이다. 반면 REST-API를 통한 다운스트림 메시지 전달시간은 1초 내외이다. 메시지 전송시간은 발신주체에서 메시지를 발신하여 수신주체가 메시지를 수신할 때까지 소요된 시간을 의미한다.
